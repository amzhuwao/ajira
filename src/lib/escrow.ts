...
export async function transitionEscrow(
  ...
  const { triggeredBy = "system", userId, reason, metadata, tx } = options;
  ...
  const run = async (client: Prisma.TransactionClient) => {
    ...
    const data: Prisma.EscrowUpdateInput = { status: toStatus };
    if (toStatus === EscrowStatus.FUNDED) data.fundedAt = new Date();
    if (toStatus === EscrowStatus.RELEASED) data.releasedAt = new Date();
